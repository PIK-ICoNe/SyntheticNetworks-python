    ...
    def set_params(self, **kwargs):
        for key in kwargs:
            if not hasattr(self, key):
                ...
                print("Possible choices: n,n0,p,q,r,s,d")
                continue
            else:
...
#            pos1 = pos0 + np.random.normal(size=2)
            pos1 = .5*pos0 + .5*np.random.uniform(low=-1,high=1,size=2)
            pos2 = self.clottiness*pos0 + (1-self.clottiness)*np.random.uniform(low=-.5,high=.5,size=2)
            return tuple(pos1 if np.random.random()<.5 else pos2)
        else:
    ...
    def _G34(self, l, node):
        ...
        if len(targets):
            ...
            if self.d[l]>0: prices /= np.array(self.density)[targets]**self.d[l]
